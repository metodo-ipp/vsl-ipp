...
const PITCH_DELAY_SECONDS = 11 * 60 + 25;
...
function GreenButton({ href, onClick }: { href: string; onClick: () => void }) {
  return (
    <>
      <style>{`
        .green-btn { display:inline-flex; align-items:center; justify-content:center; width:100%; padding:25px 46px; border-radius:10px; border:none; border-bottom:3px solid #027F20; background:radial-gradient(165.91% 647.63% at 45.92% -308.33%, #00BB2D 0%, #009624 100%); color:#fff; font-family:'Montserrat',sans-serif; font-size:18px; font-weight:900; letter-spacing:1px; text-decoration:none; text-transform:uppercase; text-align:center; position:relative; overflow:hidden; z-index:0; cursor:pointer; transition:filter .2s; }
        .green-btn:hover { filter:brightness(1.08); } .green-btn:active { translate:0 1px; }
...
export default function CidVslPage({ checkoutUrl: checkoutBaseUrl, trackingPrefix, video, metaPixelId }: CidVslPageProps) {
  ...
  useEffect(() => {
    ...
    if (video.type === "vturb") {
      ...
      const configurePitchCta = () => {
        ...
        player.displayHiddenElements(PITCH_DELAY_SECONDS, [PITCH_CTA_SELECTOR], { persist: true });
      };
  ...
  return (
    <div style={{ backgroundColor: BG, color: "#fff", fontFamily: "'DM Sans', 'Inter', sans-serif", overflowX: "hidden", minHeight: "100dvh", display: "flex", flexDirection: "column" }}>
      <style>{`
        .vsl-photo { display:block; } .vsl-content { padding:48px 32px 60px 48px; max-width:760px; } .vsl-headline,.vsl-subtitle { text-align:left; } .vsl-vturb-player,.vsl-vturb-player > vturb-smartplayer { width:100% !important; height:100% !important; border:0; display:block; } .vsl-btn { font-size:16px; letter-spacing:1.5px; white-space:nowrap; } .vsl-urgency { font-size:20px; letter-spacing:3px; white-space:nowrap; } .cid-pitch-cta { display:none; }
        @media (max-width:768px) { .vsl-photo { display:none !important; } .vsl-content { padding:36px 20px 48px; max-width:100%; } .vsl-headline,.vsl-subtitle { text-align:center; } .vsl-btn { font-size:15px; letter-spacing:.5px; white-space:normal; } .vsl-urgency { font-size:14px; letter-spacing:1.5px; } }
      `}</style>
      ...
      <section style={{ position: "relative", flex: 1, display: "flex", alignItems: "flex-start", overflow: "hidden", minHeight: "calc(100dvh - 50px)" }}>
        ...
        <div className="vsl-content" style={{ position: "relative", zIndex: 1, width: "100%" }}>
          ...
          <div aria-label="Vídeo Como Imprimir Dinheiro com Suas Palavras" style={{ borderRadius: "6px", overflow: "hidden", border: "2px solid rgba(255,215,0,.5)", boxShadow: "0 0 40px rgba(255,215,0,.08)", aspectRatio: "16/9", background: "#000", marginBottom: "28px" }}><div ref={playerElementRef} className="vsl-vturb-player" /></div>
          <div className="cid-pitch-cta"><GreenButton href={checkoutUrl} onClick={checkout} /><div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "24px", marginTop: "14px", flexWrap: "wrap" }}><span style={{ fontSize: "14px", color: "#00BB2D", fontWeight: 600 }}>✓ 7 dias de garantia</span><span style={{ fontSize: "14px", color: "#00BB2D", fontWeight: 600 }}>✓ 100% Seguro</span></div></div>
        </div>
      </section>
